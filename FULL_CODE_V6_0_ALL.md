# FULL CODE V6.0 STABLE COMPLETE EDITION
...
<script type="module" src="./app.js?v=6.0.0"></script>
...
<body class="admin-page">
  <main class="wrap admin-wrap">
    ...
    <section id="adminDashboard" class="hidden">
      <div class="admin-titlebar">
        ...
        <div>
          <a class="btn gm-zone-entry" href="./zone.html?v=6.0.0">🌙 เข้า 2D Zone · GM</a>
      ...
      <section id="usersTab" class="admin-tab-panel hidden">
        <div class="panel-title">
          ...
          <div class="admin-user-tools">
            <button id="initializeFreshDatabase" class="btn primary" type="button">🚀 เตรียมฐานข้อมูล V6.0</button>
      ...
      <section id="zoneChatLogTab" class="admin-tab-panel hidden">
        ...
        <div class="admin-world-chat-composer">
          ...
          <div class="admin-world-chat-input-row">
            ...
            <a class="btn gm-zone-entry" href="./zone.html?v=6.0.0#world-chat">เข้า World Chat</a>
          </div>
        </div>
  ...
  <script type="module" src="./admin.js?v=6.0.0"></script>
</body>
</html>
...
<body class="zone47-page">
  ...
  <script type="module" src="./zone.js?v=6.0.0"></script>
</body>
</html>
...
```html
<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="0;url=./zone.html?v=6.0.0"><title>ย้ายไป 2D Zone</title><script>location.replace("./zone.html?v=6.0.0");</script></head><body><p>ระบบแชตรวมย้ายเข้า 2D Zone แล้ว</p><a href="./zone.html?v=6.0.0">เข้า 2D Zone</a></body></html>
```
...
/* V6.0 Stable Complete Final Polish */
:root{--v6-bg:#f5f7f9;--v6-card:#fff;--v6-ink:#183246;--v6-muted:#708291;--v6-line:#dce5eb;--v6-primary:#326f9f}
body:not(.zone47-page){background:linear-gradient(180deg,#f8fafb,#f2f6f8);color:var(--v6-ink);text-rendering:optimizeLegibility}
.auth-card,.portal-card,.panel,.admin-tab-panel,.metric-card{background:var(--v6-card)!important;border-color:var(--v6-line)!important;box-shadow:0 14px 38px rgba(32,63,82,.08)!important}
.form-grid input,.form-grid select,.admin-shell input,.admin-shell select{border-radius:11px!important}
.table-wrap{border-radius:14px!important;overflow:auto!important}
thead th{position:sticky;top:0;z-index:2}
tbody tr:hover{background:#f3f8fb}
.empty{padding:22px!important;color:var(--v6-muted)!important}
...
import { getFunctions, httpsCallable } from "https://www.gstatic.com/firebasejs/12.2.1/firebase-functions.js";
import { firebaseConfig, ADMIN_USERNAME, ADMIN_EMAIL, ADMIN_UID } from "./firebase-config.js?v=6.0.0";
import { DEFAULT_MODES, DEFAULT_LEVELS } from "./default-data.js?v=6.0.0";
import { seasonIdFromDate, seasonRange, calculateRankMetrics, rankingClassKey } from "./ranking-system.js?v=6.0.0";
import { DEFAULT_TEACHER_QUESTS, clampQuestReward, questDifficultyName, questObjectiveLabel, defaultMinRankForDifficulty, rewardRange } from "./quest-system.js?v=6.0.0";
...
const adminInitializeFreshDatabase=httpsCallable(cloudFunctions,"adminInitializeFreshDatabase");
...
if($("initializeFreshDatabase"))$("initializeFreshDatabase").onclick=async()=>{
  if(!confirm("เตรียมฐานข้อมูลมาตรฐาน V6.0?\n\nระบบจะสร้าง/อัปเดต Modes, Quests และ Settings แบบ merge โดยไม่ลบข้อมูลเดิม"))return;
  const btn=$("initializeFreshDatabase"),old=btn.textContent;btn.disabled=true;btn.textContent="กำลังเตรียม...";
  try{
    const result=await adminInitializeFreshDatabase({});
    showAdminToast("ฐานข้อมูล V6.0 พร้อมใช้งาน",`Modes ${result.data?.modes||0} · Quests ${result.data?.quests||0}`);
  }catch(error){showAdminToast("เตรียมฐานข้อมูลไม่สำเร็จ",error.message||String(error),true)}
  finally{btn.disabled=false;btn.textContent=old}
};
...
import { getFunctions, httpsCallable } from "https://www.gstatic.com/firebasejs/12.2.1/firebase-functions.js";
import { firebaseConfig, ADMIN_UID } from "./firebase-config.js?v=6.0.0";
import { REWARD_ITEMS, RARITY_META, INVENTORY_CAPACITY, SHOP_BUYBACK_RATE } from "./reward-data.js?v=6.0.0";
import { DEFAULT_CHARACTER } from "./character-system.js?v=6.0.0";
import {
...
function startQuest(id){
  ...
  location.href=`./index.html?quest=${encodeURIComponent(id)}&v=6.0.0`;
}
...
export const ADMIN_USERNAME = "Pisit_2000";
export const ADMIN_EMAIL = "pisit_2000@YOUR_PROJECT.local";
...
```js
import { HTML_LEVELS } from "./levels-html.js?v=6.0.0";
import { PYTHON_LEVELS } from "./levels-python.js?v=6.0.0";
