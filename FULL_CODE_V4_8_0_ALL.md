# FULL CODE V4.8.0 NR-GAME REAL ART 2D ZONE

> PNG production assets are included under `assets/zone/` in the ZIP.
...
<head>
  ...
  <link rel="stylesheet" href="./style.css?v=4.8.0">
</head>
<body>
...
<script type="module" src="./app.js?v=4.8.0"></script>
...
<body class="admin-page">
  <main class="wrap admin-wrap">
    ...
    <section id="adminDashboard" class="hidden">
      <div class="admin-titlebar">
        ...
        <div>
          <a class="btn gm-zone-entry" href="./zone.html?v=4.8.0">🌙 เข้า 2D Zone · GM</a>
      ...
      <section id="zoneChatLogTab" class="admin-tab-panel hidden">
        ...
        <div class="admin-world-chat-composer">
          ...
          <div class="admin-world-chat-input-row">
            ...
            <a class="btn gm-zone-entry" href="./zone.html?v=4.8.0#world-chat">เข้า World Chat</a>
          </div>
        </div>
  ...
  <script type="module" src="./admin.js?v=4.8.0"></script>
</body>
</html>
...
<head>
  ...
  <link rel="stylesheet" href="./style.css?v=4.8.0">
  <link rel="preload" as="image" href="./assets/zone/zone-world-day.png">
  <link rel="preload" as="image" href="./assets/zone/male-idle-right.png">
  <link rel="preload" as="image" href="./assets/zone/female-idle-right.png">
</head>
<body class="zone47-page">
  <div id="zoneGate" class="zone47-gate">
    <div class="zone47-gate-card">
      <div class="zone47-gate-icon">🌙</div>
      <span class="section-kicker">2D SOCIAL ZONE · V4.8.0 · REAL ART</span>
  ...
  <script type="module" src="./zone.js?v=4.8.0"></script>
</body>
</html>
...
```html
<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="0;url=./zone.html?v=4.8.0"><title>ย้ายไป 2D Zone</title><script>location.replace("./zone.html?v=4.8.0");</script></head><body><p>ระบบแชตรวมย้ายเข้า 2D Zone แล้ว</p><a href="./zone.html?v=4.8.0">เข้า 2D Zone</a></body></html>
```
...
/* =====================================================================
   V4.8.0 — DUAL RANKING / PVP MULTI ROOM / TOKEN ECONOMY / ITEM SET 2
   ===================================================================== */
...
/* ==================================================================
   V4.8.0 ADMIN CLASSROOM / RANK RESET / GM WORLD CHAT / PVP COUNTDOWN
   ================================================================== */
...
/* ===== V4.8.0 SIMPLE CHAT-ONLY 2D ZONE ===== */
...
/* ==================================================================
   V4.8.0 HORIZONTAL SMOOTH ZONE + WIZARD + TOKEN SHOP
   ================================================================== */
...
/* ===== V4.8.0 PLAY STYLE + RANKED CHALLENGE ===== */
...
/* V4.8.0 Student ID 1-15 digits */
#studentId15Hint{color:#607888}


/* ===== V4.8.0 REAL ART 2D ZONE ===== */
...
import { LANGUAGES, LESSONS, DIFFICULTIES } from "./lessons.js?v=4.8.0";
import { REWARD_ITEMS, RARITY_META } from "./reward-data.js?v=4.8.0";
import { DEFAULT_CHARACTER, DEFAULT_ZONE_STATE } from "./character-system.js?v=4.8.0";
import { OFFICIAL_STAGES, OFFICIAL_TOTAL_SCORE } from "./official-data.js?v=4.8.0";
import { RANKING_CONFIG, seasonIdFromDate, seasonRange, calculateRankMetrics, rankingClassKey, rankProfiles } from "./ranking-system.js?v=4.8.0";
import { TOKEN_REWARD_CONFIG, calculateStageTokenReward, maxTokenForLesson, classKey } from "./economy-system.js?v=4.8.0";
import { DEFAULT_TEACHER_QUESTS, localDayKey, questObjectiveMet, questObjectiveLabel, clampQuestReward } from "./quest-system.js?v=4.8.0";
...
async function routeAuthenticatedStudent(){
  ...
  if(isMobileOrTabletDevice() && ["male","female"].includes(state.player?.character?.gender)){
    ...
    location.replace("./zone.html?v=4.8.0");
    return;
  }
...
$("questZoneButton").onclick=()=>{location.href="./zone.html?v=4.8.0"};
...
/* ===== V4.8.0 PVP MULTI ROOM · 1/3/5 SHOT · 1V1/2V2 RELAY · TOKEN WAGER ===== */
...
onAuthStateChanged(auth,async user=>{
  ...
  if(user.email==="pisit_2000@nr-game-code.local"){location.replace("./admin.html?v=4.8.0");return;}
  state.uid=user.uid;
  try{
    await routeAuthenticatedStudent();
  }catch(error){
...
import { firebaseConfig, ADMIN_USERNAME, ADMIN_EMAIL, ADMIN_UID } from "./firebase-config.js?v=4.8.0";
import { DEFAULT_MODES, DEFAULT_LEVELS } from "./default-data.js?v=4.8.0";
import { seasonIdFromDate, seasonRange, calculateRankMetrics, rankingClassKey } from "./ranking-system.js?v=4.8.0";
import { DEFAULT_TEACHER_QUESTS, clampQuestReward, questDifficultyName, questObjectiveLabel, defaultMinRankForDifficulty, rewardRange } from "./quest-system.js?v=4.8.0";
...
function shopArtForItem(item){
  ...
  if(item?.slot==="hand")return ZONE_ART_PATH.scroll;
  if(item?.slot==="aura")return ZONE_ART_PATH.gem;
  if(item?.slot==="pet")return ZONE_ART_PATH.potionGreen;
  const rarity=String(item?.rarity||"common");
  return rarity==="mythic"?ZONE_ART_PATH.chest:rarity==="legendary"?ZONE_ART_PATH.gem:rarity==="epic"?ZONE_ART_PATH.potionBlue:rarity==="rare"?ZONE_ART_PATH.potionGreen:ZONE_ART_PATH.potionRed;
}
...
function renderShop(){
  ...
  $("zoneShopGrid").innerHTML=items.map(item=>{
    ...
    return `<article class="zone47-shop-item rarity-${esc(item.rarity)} ${on?"wearing":""}">
      ...
      <div class="zone47-shop-icon zone47-shop-real-art"><img src="${shopArtForItem(item)}" alt=""><span>${item.icon}</span></div><strong>${esc(item.name)}</strong><small>${esc(item.description)}</small>
...
function drawEquipmentFx(c,p,now){
  if(isGMPlayer(p))return;
  const eq=equipped(p.character||{}),aura=itemById(eq.aura),pet=itemById(eq.pet),hand=itemById(eq.hand),back=itemById(eq.back);
  if(aura){c.save();c.globalAlpha=.44;c.strokeStyle=itemColor(aura);c.lineWidth=5;c.beginPath();c.ellipse(0,-65,54,83,0,0,Math.PI*2);c.stroke();c.restore()}
  if(back){c.save();c.font="27px system-ui";c.textAlign="center";c.fillText(back.icon||"🎒",-48,-38);c.restore()}
  if(hand){c.save();c.font="26px system-ui";c.textAlign="center";c.fillText(hand.icon||"✨",48,-44);c.restore()}
  if(pet){c.save();c.font="29px system-ui";c.textAlign="center";c.fillText(pet.icon||"🐾",62,5+Math.sin(now/260)*4);c.restore()}
}
...
```js
import { HTML_LEVELS } from "./levels-html.js?v=4.8.0";
import { PYTHON_LEVELS } from "./levels-python.js?v=4.8.0";
